def PasumoOCR(image_file):
    ...
    #img = cv2.merge((img_bgr[2],img_bgr[2],img_bgr[2]))
    # detect pink 
    ...
    upper = np.array([250, 250, 220])#ここまでの範囲を指定することで、青い文字以外を指定する。
    img_mask = cv2.inRange(hsv, lower, upper)
    ...
    white_pixels = (img == (0, 0, 0)).all(axis=-1)#背景の黒色を(0, 0, 0)、と指定
    img[white_pixels] = (160,162,162)#ここで黒色(0, 0, 0)の範囲を背景の色(241,242,242)に、変更している。
    cv2.imwrite("end.png", img)#おそらく、ここで変更された画像をセーブしている。
    ...
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    lower = np.array([0, 0, 10])    #真っ黒
    upper = np.array([220, 250, 210])#(現状)青の文字のみが読み取れる範囲指定
    img_mask = cv2.inRange(hsv, lower, upper)
    img_color = cv2.bitwise_and(img, img, mask=img_mask)
    cv2.imwrite("end.png", img_color)#黒文字になったのを保存。これで画像が完成。
    ...
    box_area = np.array( [[10, 40, 190, 35],   #START
                        [225, 40, 400, 35],  #END
