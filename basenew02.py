...
def dow():
    ...
    canvas02b = tk.Canvas(root, width=300, height=250)
    canvas02b.create_rectangle(0, 0, 949,333, fill='black')
    canvas02b.place(x=320, y=30)
    canvas02 = tk.Canvas(root, width=290, height=240)
    canvas02.create_rectangle(0, 0, 0, 0, fill='gray')
    canvas02.place(x=325, y=35)

    canvas03b = tk.Canvas(root, width=310, height=360)
    canvas03b.create_rectangle(0, 0, 310,360, fill='black')
    canvas03b.place(x=645, y=30)
    canvas03 = tk.Canvas(root, width=300, height=350)
    canvas03.create_rectangle(0, 0, 0, 0, fill='gray')
    canvas03.place(x=650, y=35)

    canvas02b = tk.Canvas(root, width=611, height=95)
    canvas02b.create_rectangle(0, 0, 949,333, fill='black')
    canvas02b.place(x=10, y=295)
    canvas02 = tk.Canvas(root, width=601, height=85)
    canvas02.create_rectangle(0, 0, 0, 0, fill='gray')
    canvas02.place(x=15, y=300)

    canvas05b = tk.Canvas(root, width=948, height=167)
    canvas05b.create_rectangle(0, 0, 949,167, fill='black')
    canvas05b.place(x=10, y=420)
    canvas02 = tk.Canvas(root, width=938, height=157)
    canvas02.create_rectangle(0, 0, 0, 0, fill='gray')
...
#pack
    ...
    button = tk.Button(text="ターミナルに表示",command=lambda:print(combo.get()+"\n",combo2.get()))#\nで改行,それぞれのcomboの値を表示
    # ボタンの配置
    button.grid(column=50, row=20)
...
#width=935, height=150)
    ...
    text_widget = tk.Text(root, width=134, height=12)
    text_widget.grid()
    text_widget.place(x=15, y=425)
